...
def calc_2d_metric(rec_meshfile, gt_meshfile, n_imgs=1000):
    """
...
def eval_recon(rec_mesh, gt_mesh, eval_2d, eval_3d, distance_thresh=0.01):
    result = {}
    try:
        if eval_3d:
            ...
            result_3d = run_evaluation(pred_ply, path_to_pred_ply, gt_mesh.split("/")[-1][:-4],
                                       distance_thresh=distance_thresh, full_path_to_gt_ply=gt_mesh, icp_align=True)
            result = dict(result.items() | result_3d.items())
        if eval_2d:
            result_2d = calc_2d_metric(
                rec_mesh, gt_mesh, n_imgs=10)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--distance_thresh", type=float, default=0.005)
    args = parser.parse_args()

    result = eval_recon(args.rec_mesh, args.gt_mesh, args.eval_2d, args.eval_3d, distance_thresh=args.distance_thresh)
    
    log_info = (
        f'F1: {result["f-score"]:.4f}, '
        f'Prec: {result["precision"]:.4f}, '
        f'Recall: {result["recall"]:.4f}, '
        f'MeanPrec: {result["mean precision"] * 1000:.4f} mm, '
        f'MeanRecall: {result["mean recall"] * 1000:.4f} mm, '
        f'd_th: {result["dist_threshold"]} m'
    )
    save_path = os.path.join(os.path.dirname(args.rec_mesh), 'results.txt')
